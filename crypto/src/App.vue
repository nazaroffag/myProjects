<template>
  ...
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
      ...
      <!-- прелоадер -->
    <div v-if="!tList" class="fixed w-100 h-100 opacity-80 bg-blue-500 inset-0 z-50 flex items-center justify-center">
    ...
    <!-- инпут Тикер -->
      <section>
          <div class="flex flex-col">
            <div class="max-w-xs">
              ...
              <div v-if="tickPop.length" class="flex border-b-2 border-gray-600 mt-4 mb-2 pb-4">
                  <div v-for="(tick, idx) in tickPop" v-bind:key="idx" v-on:click.stop="addToInpt(tick)" class="text-gray-700 bg-gray-400 rounded-lg px-3 cursor-pointer mr-2">{{ tick }}</div>
              </div>
              <div v-if="tickerCheck" class="text-red-700">Такой тикер уже добавлен</div>
            </div>
          </div>
     ...
     <template v-if="tickers.length">
      ...
      <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3" >
        ...
        <div v-for="(t,idx) in paginatedTickers" v-bind:key="t.name" v-on:click="select(t)" v-bind:class="{ 'border-4' : selectedTicker == t }"
        ...
        >
          ...
          <button v-on:click.stop="delTicker(idx)" 
      ...
      <div class="flex items-end border-gray-600 border-b border-l h-64">
        <div
          v-for="(bar, idx) in normalizedGraph" :key="idx" :style="{ height: `${bar}%` }"
          class="bg-purple-800 border w-10"
        ></div>
      </div>
      <button v-on:click="selectedTicker = null"
        type="button"
