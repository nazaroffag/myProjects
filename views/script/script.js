...
for (let i = 0; i < base.length; i++) {
  let monthNum = base[i].date.toString().substr(5, 2);
  let day = base[i].date.toString().substr(8, 2);
  let date = day + " " + month[monthNum];
  dateArray.push(date);
  tempArray.push({ baseIndex: i, date: date, product: base[i] });
  operationArray.push(base[i].id);
}
...
operationArray = unique(operationArray);
  ...
  for (let y = 0; y < tempArray.length; y++) {
    if (dateArray[i] === tempArray[y].date) {
      ...
      for (let i = 0; i < operationArray.length; i++) {
        ...
        let operationSum = 0;
        for (let y = 0; y < tempArray.length; y++) {
          if (operationArray[i] === tempArray[y].product.id) {
            products.push(tempArray[y]);
            operationSum += Number(tempArray[y].product.price);
          }
        }
        dateOperations.push({
          operationNum: operationArray[i],
